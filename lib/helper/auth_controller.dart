...
class AuthController {
  ...
  Future signOut() async {
    try {
      updateUserById(getCurrentUser()!.uid, {'token': ''});
  ...
  Future<String?> createUserFromEmail(
    ...
    try {
      var result = await signUpEmail(userModel.getEmail(), password);
      if (result is User) {
  ...
  Future<String?> createUserFromGoogleSignIn(User user) async {
    ...
    try {
      ...
      UserModel userModel = UserModel(
          ...
          '',
          '');
      await createUserById(user.uid, userModel.toJson());
      return null;
    } on FirebaseAuthException catch (e) {
      return e.toString();
    }
  ...
  Future<bool> checkUserExist(String id) async {
    var result = await getUserFromId(id);
